...
const BookingScreen = () => {
  ...
  const handlePayment = async () => {
    try {
      // Lưu thông tin vé đặt lên Firestore
  ...
  const renderSeats = () => {
    return allSeats.map(seat => {
      // Kiểm tra trạng thái ghế đã chọn từ dữ liệu đã lưu trên Firestore
      const isSeatSelected = previousBookings.some(booking => 
      ...
      return (
        <TouchableOpacity
          ...
          style={[
            ...
            isSeatSelected && styles.lockedSeat, // Thêm kiểm tra trạng thái ghế đã chọn
            selectedSeats.some(
              selectedSeat =>
