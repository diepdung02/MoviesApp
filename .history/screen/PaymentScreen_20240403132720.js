...
const PaymentScreen = () => {
  ...
  const savePaymentToFirestore = () => {
    const user = firebase.auth().currentUser; // Lấy thông tin người dùng hiện đang đăng nhập
    if (user) {
      ...
      firestore()
        .collection('users')
        .doc(uid) // Tài liệu Firestore của người dùng được xác định bằng UID
        .collection('payments')
        .add(ticket) // Thêm dữ liệu thanh toán vào collection của người dùng
        .then(() => {
          console.log(
            'Dữ liệu thanh toán đã được lưu trữ thành công trên Firestore!',
          );
          setPaymentCompleted(true); // Đánh dấu thanh toán đã hoàn thành
        })
        .catch(error => {
          console.error(
