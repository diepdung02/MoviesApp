...
const BookingScreen = () => {
  ...
  useEffect(() => {
    const unsubscribe = firestore()
      ...
      .catch(error => console.error('Error getting reserved seats:', error));
  ...
  const handlePayment = () => {
    ...
    selectedSeats.forEach(seat => {
      ...
      // Thêm thông tin vé vào collection 'tickets' của người dùng
      firestore()
        .collection('users')
        .doc(userEmail)
        .collection('tickets')
        .add(ticketData)
        .then(() => console.log('Ticket added to user successfully'))
        .catch(error => console.error('Error adding ticket to user:', error));

      // Thêm thông tin vé vào collection 'ticket'
      firestore()
