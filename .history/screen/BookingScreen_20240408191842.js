...
const BookingScreen = () => {
  ...
  useEffect(() => {
    const unsubscribe = firestore()
      ...
      .onSnapshot(querySnapshot => {
        ...
        // Cập nhật trạng thái ghế đã đặt chỗ từ dữ liệu trước đó
        const updatedSeats = allSeats.map(seat => {
          const isBooked = data.some(booking => booking.seats.includes(`${seat.row}${seat.number}`));
          return { ...seat, isLocked: isBooked };
        });
  ...
  const handleSeatSelection = seat => {
    if (!Array.isArray(selectedSeats)) {
      console.error('SelectedSeats is not an array.');
      return;
    }
