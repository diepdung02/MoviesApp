...
const PaymentScreen = () => {
  ...
  const { movieTitle, selectedSeats, movieIMG, selectedShowtime, totalPrice } = route.params;
  ...
  const savePaymentToFirestore = () => {
    ...
    if (user) {
      ...
      const ticket = {
        ...
        selectedDate: new Date().toISOString().split('T')[0], // Lấy ngày hiện tại
      };
      ...
      {paymentCompleted ? (
        <View>
          ...
          <View style={styles.paymentDetails}>
            ...
            <Text style={styles.detailText}>{new Date().toISOString().split('T')[0]}</Text>
