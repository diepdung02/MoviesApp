  ...
  const savePaymentToFirestore = () => {
    ...
    if (user) {
      ...
      firestore()
        ...
        .doc(uid)
        .collection('payments')
        .doc() // Để tạo một tài liệu mới với ID tự động
        .set(ticket) // Sử dụng phương thức set() để cập nhật hoặc tạo mới tài liệu
        .then(() => {
          console.log('Dữ liệu thanh toán đã được lưu trữ thành công trên Firestore!');
