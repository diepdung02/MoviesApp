...
const BookingScreen = () => {
  ...
  // Load saved seats from AsyncStorage when component mounts
  useEffect(() => {
    const loadSavedSeats = async () => {
      try {
        const savedSeats = await AsyncStorage.getItem('selectedSeats');
        if (savedSeats) {
  ...
  // Save selected seats to AsyncStorage whenever it changes
  useEffect(() => {
