...
const MyTicketsScreen = () => {
  ...
  const renderTicketItem = ({ item }) => (
    <View style={styles.ticketItem}>
      ...
      <Text style={styles.ticketText}>🎟 Seats: {renderSeatRows(item.selectedSeats)}</Text>
      <Text style={styles.ticketText}>⏰ Showtime: {item.selectedShowtime}</Text>
      <Text style={styles.ticketText}>💰 Total Price: {getFormattedPrice(item.totalPrice)}</Text>
    </View>
  );
  function renderSeatRows(seats) {
    ...
    seats.forEach(seat => {
      ...
      seatRows[row].push(seatNumber);
    });
  
    // Render seat rows
    return seatRows.map((row, index) => (
      <Text key={index}>H{index + 1}: {row.join(', ')}</Text>
    ));
  }
