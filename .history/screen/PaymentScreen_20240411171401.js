...
const PaymentScreen = () => {
  ...
  const savePaymentToFirestore = () => {
    ...
    if (user) {
      ...
      const bookingDate = new Date().toISOString(); // Lấy ngày đặt vé hiện tại
  
      firestore()
        .collection('users')
        .doc(email)
        .collection('tickets')
        .doc(bookingDate) // Sử dụng ngày đặt vé làm ID cho tài liệu
        .set(ticket)
        .then(() => {
          console.log('Dữ liệu thanh toán đã được lưu trữ thành công trên Firestore với ID:', bookingDate);
