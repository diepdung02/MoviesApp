...
const BookingScreen = () => {
  ...
  const handlePayment = async () => {
    try {
      // Lưu thông tin về những ghế đã chọn vào Firestore
  ...
  const isSeatLocked = (seat) => {
    ...
    // Kiểm tra xem ghế đã chọn có bị khóa không
    const isSeatLocked = firestore().collection('selectedSeats').where('row', '==', seat.row).where('number', '==', seat.number).get().then(querySnapshot => !querySnapshot.empty);
  
    return isSeatSelected && isSeatLocked;
  };
  
  // Sử dụng isSeatLocked để kiểm tra và khóa ghế khi hiển thị danh sách ghế
  const renderSeats = () => {
    return allSeats.map(seat => (
      <TouchableOpacity
        ...
        style={[
          ...
          isSeatLocked(seat) && styles.lockedSeat,
        ]}
        onPress={() => handleSeatSelection(seat)}
        disabled={isSeatLocked(seat)}>
        <Text
          style={[
            isSeatLocked(seat) && styles.lockedSeatText,
            selectedSeats.some(
              selectedSeat =>
                selectedSeat.row === seat.row &&
                selectedSeat.number === seat.number,
