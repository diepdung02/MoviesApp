...
const BookingScreen = () => {
    ...
    const unsubscribe = firestore()
      ...
      .onSnapshot(querySnapshot => {
        ...
        querySnapshot.forEach(doc => {
          ...
          const { row, number } = seatData;
          const booked = selectedSeats.includes(`${row}${number}`); // Kiểm tra xem ghế đã được đặt hay chưa
          seats.push({ row, number, type: getType(row), booked });
        });
        setAllSeats(seats);
      });
  
    return () => unsubscribe();
  }, [selectedSeats]); // Thêm selectedSeats vào mảng dependency để hàm này được gọi lại khi có thay đổi trong selectedSeats
  ...
  const handleSeatSelection = async (row, number) => {
    ...
    if (isSeatAvailable) {
      ...
      // Hiển thị thông báo rằng ghế ngồi đã được đặt
