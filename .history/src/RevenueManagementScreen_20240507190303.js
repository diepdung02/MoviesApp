...
const RevenueScreen = () => {
  ...
  useEffect(() => {
    const unsubscribe = firestore()
      ...
      // Truy vấn vào collection "movieId" để lấy tên phim từ movieId
      const movieRef = firestore().collection('ticket').doc(doc.id).collection('movieId');
      const movieDoc = await movieRef.get();
      let movieTitle = 'Unknown Movie';
      if (!movieDoc.empty) {
        movieDoc.forEach(movie => {
