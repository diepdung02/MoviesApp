...
const RevenueScreen = () => {
  ...
  // Hàm renderRevenueItem và phần JSX như trước
}
Bằng cách này, bạn sẽ tính tổng doanh thu từng ngày và tổng doanh thu từng tháng một cách riêng biệt, giúp hiển thị chính xác doanh thu hàng tháng.
...
log ra
ChatGPT
Dưới đây là cách thêm log vào mã của bạn để hiển thị thông tin doanh thu hàng ngày và hàng tháng:

javascript
Copy code
const RevenueScreen = () => {
  // Các state và useEffect như trước
  ...
  useEffect(() => {
    const unsubscribeDaily = firestore()
      ...
      .onSnapshot(querySnapshot => {
        ...
        querySnapshot.forEach(doc => {
          const { selectedDate, totalPrice } = doc.data();
          console.log("Selected Date:", selectedDate); // Log ngày được chọn
