  ...
  const calculateTotalRevenue = async () => {
    try {
      ...
      querySnapshot.forEach(doc => {
        ...
        console.log('Ticket Date:', ticketDate); // Log giá trị ngày để kiểm tra
        const dayKey = ticketDate.toISOString().split('T')[0]; // Lấy ngày (yyyy-mm-dd)
        const monthKey = ticketDate.toISOString().slice(0, 7); // Lấy tháng (yyyy-mm)
