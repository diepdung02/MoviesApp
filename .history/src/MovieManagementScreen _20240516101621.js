...
const MovieManagementScreen = () => {
  ...
  const uploadImage = async (uri) => {
    const filename = title.trim().replace(/\s+/g, '-') + '-' + new Date().getTime(); // Tạo tên file từ tiêu đề phim và thời gian hiện tại
  ...
  return (
    <KeyboardAwareScrollView style={{ flex: 1 }}>
      <View style={styles.container}>
        ...
        {images ? <Image source={{ uri: images }} style={styles.selectedImage} /> : null}
        <Picker
