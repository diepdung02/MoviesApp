...
const RevenueScreen = () => {
  ...
  useEffect(() => {
      ...
      .onSnapshot(querySnapshot => {
        ...
        querySnapshot.forEach(doc => {
          ...
          if (isValid(date)) {
            ...
            const formattedMonth = format(date, 'yyyy-MM'); // Định dạng ngày tháng thành "yyyy-MM"
    ...
    const unsubscribeMonthly = firestore()
      ...
      .orderBy('selectedDate', 'desc')
      .onSnapshot(querySnapshot => {
        // Code xử lý dữ liệu doanh thu hàng tháng
      });

    return () => {
      unsubscribeDaily();
      unsubscribeMonthly(); // Unsubscribe cả mảng monthlyRevenue
    };
  }, []);
