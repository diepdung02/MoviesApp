  ...
  const calculateTotalRevenue = async () => {
    try {
      ...
      querySnapshot.forEach(doc => {
       ...
       // Chuyển đổi selectedDate thành đối tượng Date
const dateParts = ticketData.selectedDate.split('T')[0].split('-');
const ticketDate = new Date(dateParts[0], dateParts[1] - 1, dateParts[2]); // Subtract 1 from month because months are zero-indexed
        ...
        // Lấy ngày (yyyy-mm-dd)
        const dayKey = ticketDate.getTime();
        // Lấy tháng (yyyy-mm)
        const monthKey = ticketDate.getMonth();
